...
$watchdogState = readJsonFile(getRestartLogPath(), [
    'last_restart' => 0,
    'restarts' => [],
...
if (is_file($fallbackFile)) $fallbackText = trim((string)file_get_contents($fallbackFile));

function typeLabel(string $type): string {
    $map = ['clock'=>'Uhr','image'=>'Bild','video'=>'Video','website'=>'Webseite'];
...
<div class="card">
  ...
  <form action="save_settings.php" method="post" enctype="multipart/form-data">
    <div class="grid">
      <div><label>Standarddauer</label><input type="number" name="defaultDuration" min="1" value="<?= (int)($config['player']['defaultDuration'] ?? 8) ?>"></div>
      <div><label>Bild-Fade</label><input type="number" step="0.1" name="imageFade" min="0" value="<?= h((string)($config['player']['imageFade'] ?? 1.2)) ?>"></div>
      <div><label>Bild-Anpassung</label><select name="fit"><option value="contain" <?= ($config['player']['fit'] ?? '') === 'contain' ? 'selected' : '' ?>>contain</option><option value="cover" <?= ($config['player']['fit'] ?? '') === 'cover' ? 'selected' : '' ?>>cover</option></select></div>
      <div><label>Hintergrundfarbe</label><input type="color" name="background" value="<?= h($config['player']['background'] ?? '#ffffff') ?>"></div>
      <div><label>Video-Modus</label><select name="videoMode"><option value="until_end" <?= ($config['player']['videoMode'] ?? '') === 'until_end' ? 'selected' : '' ?>>bis Ende</option><option value="fixed" <?= ($config['player']['videoMode'] ?? '') === 'fixed' ? 'selected' : '' ?>>feste Dauer</option></select></div>
      <div><label>Videos stumm starten</label><select name="startMuted"><option value="1" <?= !empty($config['player']['startMuted']) ? 'selected' : '' ?>>Ja</option><option value="0" <?= empty($config['player']['startMuted']) ? 'selected' : '' ?>>Nein</option></select></div>
    </div>
    ...
    <div class="grid">
      ...
      <div><label>Uhr Dauer</label><input type="number" name="clockDuration" min="1" value="<?= (int)($config['clock']['duration'] ?? 10) ?>"></div>
    ...
    <div class="grid">
      ...
      <div><label>CPU Limit %</label><input type="number" name="cpuLimit" min="1" max="100" value="<?= (int)($config['system']['cpuLimit'] ?? 85) ?>"></div>
      <div><label>RAM Limit %</label><input type="number" name="ramLimit" min="1" max="100" value="<?= (int)($config['system']['ramLimit'] ?? 85) ?>"></div>
      <div><label>Cooldown Sekunden</label><input type="number" name="cooldownSeconds" min="30" value="<?= (int)($config['system']['cooldownSeconds'] ?? 180) ?>"></div>
      <div><label>Max Restarts / 30 Min</label><input type="number" name="maxRestartsIn30Min" min="1" value="<?= (int)($config['system']['maxRestartsIn30Min'] ?? 3) ?>"></div>
...
<div class="card">
  ...
  <form action="upload.php" method="post" enctype="multipart/form-data">
    <div class="grid">
      <div><label>Datei</label><input type="file" name="mediaFile" accept=".png,.jpg,.jpeg,.webp,.gif,.mp4,.webm,.mov"></div>
      <div><label>Titel</label><input type="text" name="title" placeholder="z. B. Empfang"></div>
      <div><label>Typ</label><select name="type"><option value="image">Bild</option><option value="video">Video</option></select></div>
      <div><label>Dauer</label><input type="number" name="duration" min="1" value="<?= (int)($config['player']['defaultDuration'] ?? 8) ?>"></div>
...
<div class="card"><h2>Playlist</h2><table>
<thead><tr><th>Sort</th><th>Titel</th><th>Typ</th><th>Status</th><th>Info</th><th>Aktionen</th></tr></thead><tbody>
...
<td><?php if (($item['type'] ?? '') === 'website'): ?><code><?= h($item['url'] ?? '') ?></code><?php else: ?><code><?= h($item['file'] ?? '') ?></code><?php endif; ?><div class="small">Dauer: <?= (int)($item['duration'] ?? 0) ?>s</div></td>
...
<tr id="edit-<?= h($item['id'] ?? '') ?>" class="editRow"><td colspan="6">
