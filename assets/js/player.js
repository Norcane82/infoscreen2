(() => {
  ...
  function makeBaseSlide(slide) {
    ...
    wrapper.style.transitionDuration = `${Math.max(0.12, Number(slide.fade || defaultFade))}s, 0s`;
  ...
  function updateClock(node) {
    ...
    node._clockElements.timeEl.textContent = new Intl.DateTimeFormat('de-AT', timeOptions).format(now);
    node._clockElements.dateEl.textContent = new Intl.DateTimeFormat('de-AT', {
      weekday: 'long',
      day: '2-digit',
      month: '2-digit',
      year: 'numeric',
  ...
  function activateNode(node, slide) {
    ...
    if (slide.type === 'clock') {
      updateClock(node);
      clockInterval = window.setInterval(() => updateClock(node), 1000);
    }
  ...
  function removeOldNode(oldNode, fadeSeconds) {
    if (!oldNode) {
      return;
    }
    ...
    oldNode.style.transitionDuration = `${Math.max(0.12, Number(fadeSeconds || defaultFade))}s, 0s`;

    window.setTimeout(() => {
      oldNode.remove();
    }, Math.max(350, Number(fadeSeconds || defaultFade) * 1000 + 100));
  }

  function finishTransition(token, oldNode, slide) {
    const fadeSeconds = Math.max(0.12, Number(slide.fade || defaultFade));
    const fadeOutMs = Math.max(120, Math.round(fadeSeconds * 350));
    ...
    window.setTimeout(() => {
      ...
      window.setTimeout(() => {
        ...
        removeOldNode(oldNode, fadeSeconds);
  ...
  function showSlide(index) {
    ...
    const fadeSeconds = Math.max(0.12, Number(slide.fade || defaultFade));
    const settleMs = Math.max(100, Math.round(fadeSeconds * 180));
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        ...
        window.setTimeout(() => {
          ...
          finishTransition(token, oldNode, slide);
        }, settleMs);
      });
    });
